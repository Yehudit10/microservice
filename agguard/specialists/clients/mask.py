...
from agguard.specialists.mask_classifier import MaskPrediction  # dataclass

log = logging.getLogger(__name__)
Box = Tuple[int, int, int, int]

# tolerate either service name in compiled stubs
...
class GrpcMaskClassifierClient:
    def __init__(self, address: str, model_name: str = "mask",
                 timeout_sec: float = 1.5, jpeg_quality: int = 85):
        ...
        log.info("GrpcMaskClassifierClient -> %s (model=%s)", self.address, self.model_name)
